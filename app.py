...
def set_cell_paddings(cell, top=None, start=None, bottom=None, end=None):
    """Menambah ruang dalam (padding) pada sel jadual dengan namespace XML yang betul"""
    tc = cell._tc
...
def generate_docx(matrix_df, col_sums, wabak_df, vector_df, bkk_df, bkk_count):
    ...
    h21_text = f"Jadual di bawah menunjukkan jumlah wabak harian dan kumulatif... Sejumlah {harian_total} wabak diterima pada {get_malay_date(yesterday)}."
    ...
    for i, (peny, row_data) in enumerate(wabak_df.iterrows()):
        ...
        for j in [1, 2]:
            ...
            apply_font(row[j].paragraphs[0].add_run(str(int(row_data['HARIAN' if j==1 else 'KUMULATIF']))), 8, bold=True)

    # Section 3.0 rapat di bawah 2.0
    doc.add_paragraph()
    apply_font(doc.add_paragraph().add_run("3.0 Ringkasan Laporan Wabak Vektor"), 11, bold=True)
    # ... (Logik Jadual 3 diringkaskan, gunakan lebar Inches(2.3) untuk DAERAH)
    ...
    h41_text = f"4.1 Jadual di bawah menunjukkan jumlah kejadian insiden... Sejumlah {bkk_count} input telah diterima pada {get_malay_date(yesterday)}." if bkk_count > 0 else f"4.1 Tiada Kejadian Insiden dilaporkan pada {get_malay_date(yesterday)}."
    ...
    for i, txt in enumerate(h4_headers):
        ...
        set_cell_paddings(cell, top=60, bottom=60)
    ...
    # Baris Jumlah Akhir Jadual 4
    f_row = t4.rows[-1].cells
    apply_font(f_row[0].paragraphs[0].add_run("JUMLAH"), 7.5, bold=True)
    set_cell_background(f_row[0], "FFFF00")
    for i in range(1, len(h4_headers)):
        set_cell_background(f_row[i], "FFFF00")
        p = f_row[i].paragraphs[0]
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        # Ambil column sum secara dinamik
        col_name = h4_headers[i].replace("\n", " ")
        if "DIISYTIHAR" in col_name: col_name = 'KKM_DECLARE'
        val = bkk_df[col_name].sum() if col_name in bkk_df.columns else 0
        apply_font(p.add_run(str(int(val))), 8, bold=True)
...
if f1 and f2:
    if st.button("🚀 Jana Laporan Penuh (1.0 - 4.0)"):
        try:
            ...
            doc_out = generate_docx(matrix, col_totals, None, None, matrix_bkk, bkk_count)
