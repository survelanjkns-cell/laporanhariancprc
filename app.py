...
# Specific URL for the Trend Graph data
...
def capture_dengue_trend_graph():
    """Fetches numerical data from B:BB and recreates the trend graph visually."""
    try:
        # Load the sheet. header=None to look for labels in Column A manually
        df_trend = pd.read_csv(GRAF_DATA_URL, header=None)
        labels = df_trend.iloc[:, 0].astype(str).str.strip()
        
        # Identify rows by searching the first column
        try:
            ...
            # Best guess positions based on typical sheet layout
            idx_2025, idx_2026, idx_median = 5, 6, 7 

        weeks = range(1, 54) # Weeks 1 to 53
        
        # Column B to BB is index 1 to 53
        ...
        plt.plot(weeks, y2026, label='2026', color='#C00000', linewidth=2.5) # Darker red for emphasis
...
def generate_docx(matrix_df, col_sums, wabak_df, vector_df, bkk_table_df, is_bkk_empty, bkk_details, df_yesterday_list, trend_graph):
    ...
    # --- NEW: TREND GRAPH INSERTION (RAJAH 1) ---
    if trend_graph:
...
# --- TREND GRAPH CAPTURE HELPER ---
def capture_dengue_trend_graph():
    """Fetches numerical data from B:BB and recreates the trend graph."""
    try:
        df_trend = pd.read_csv(GRAF_DATA_URL, header=None)
        labels = df_trend.iloc[:, 0].astype(str).str.strip()
...
if f1 and f2:
    if st.button("🚀 Jana Laporan Lengkap"):
        try:
            ...
            addr_col = 'Tempat Berlaku Wabak\n(Alamat diisi lengkap with :- No rumah, nama jalan, nama tempat, daerah dan Negeri)'
            ...
            doc_out = generate_docx(matrix, col_totals, wabak_df, v_data, bkk_table_final, (len(bkk_details)==0), bkk_details, df_yesterday_list, trend_img)
