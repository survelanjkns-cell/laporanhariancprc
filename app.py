...
# --- NEW: GRAPH IMAGE GENERATOR ---
def get_graph_image():
    try:
        # Load published CSV, skip title row
        ...
        if median_col:
            fig.add_trace(go.Scatter(x=df_plot.index, y=df_plot[median_col[0]], mode='lines', name='Moving Median (4 thn)', line=dict(color='#FBBC04', width=2)))
        ...
        # Capture as PNG bytes (Requires kaleido library)
...
def generate_docx(matrix_df, col_sums, wabak_df, vector_df, bkk_table_final, is_bkk_empty, bkk_details, df_yesterday_list):
    ...
    if not df_yesterday_list:
        ...
        for idx, item in enumerate(df_yesterday_list, start=1):
            row = t21.add_row().cells
            row[0].text = str(idx)
            row[1].text = f"{item[0]}\n({'(Household)' if str(item[3]) == 'Rumah Persendirian' else '(Institusi)'})"
            ...
            ar = f"({(n_kes/n_dedah)*100:.1f}%)" if n_dedah > 0 else "(0%)"
            row[4].text = f"{int(n_kes)}/{int(n_dedah)}\n{ar}"
            for c in range(5):
                row[c].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.LEFT if c==3 else WD_ALIGN_PARAGRAPH.CENTER
    ...
    h31_text = f"3.1 Jadual di bawah menunjukkan jumlah wabak vektor harian dan kumulatif di negeri Selangor. {xx_v if xx_v>0 else 'Tiada'} input notifikasi wabak vektor telah diterima pada {get_malay_date(yesterday)}."
    ...
    for i in [0, 1, 3, 5]:
        ...
        apply_font(h3_r1[i].paragraphs[0].runs[0], 10, bold=True)
    ...
    for i in range(len(vector_df)):
        ...
        for j in range(7):
            ...
            txt = str(val).title() if j==0 else (f"{int(float(val)):,}" if pd.notna(val) else "-")
            p = row_cells[j].paragraphs[0]
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT if j==0 else WD_ALIGN_PARAGRAPH.CENTER
            apply_font(p.add_run(txt), 9, bold=True)
            if "JUMLAH" in str(vector_df.iloc[i, 0]).upper(): set_cell_background(row_cells[j], "FFFF00")
            elif j==0: set_cell_background(row_cells[j], "FCE4D6")

    # --- NEW: INTEGRATED GRAPH SECTION ---
    doc.add_paragraph() # Spacer
    graph_img = get_graph_image()
    if graph_img:
        p_graph = doc.add_paragraph()
        p_graph.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run_graph = p_graph.add_run()
        run_graph.add_picture(graph_img, width=Inches(6.2))
        
        # Graph Caption
        p_cap = doc.add_paragraph()
        p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run_cap = p_cap.add_run("Rajah 1 : Carta Kes Mingguan Denggi Didaftarkan Bagi Tahun 2025-2026 Negeri Selangor")
        apply_font(run_cap, 9, bold=True)
    ...
    if is_bkk_empty: h41_text = f"4.1 Jadual di bawah menunjukkan jumlah kejadian insiden bencana, kecemasan dan krisis (BKK) di negeri Selangor. Tiada insiden dilaporkan pada {get_malay_date(yesterday)}."
    else: h41_text = f"4.1 Jadual di bawah menunjukkan jumlah kejadian insiden bencana, kecemasan dan krisis (BKK) di negeri Selangor. Terdapat {len(bkk_details)} kejadian dilaporkan pada {get_malay_date(yesterday)}."
    ...
    t4 = doc.add_table(rows=len(bkk_table_final) + 1, cols=len(bkk_table_final.columns))
    t4.style = 'Table Grid'
    for i, col in enumerate(bkk_table_final.columns):
        cell = t4.rows[0].cells[i]
        set_cell_background(cell, "BFDFFF")
        p = cell.paragraphs[0]
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        apply_font(p.add_run(str(col).strip()), 8, bold=True)
    for r_idx, row_data in enumerate(bkk_table_final.values):
        cells = t4.rows[r_idx+1].cells
        for c_idx, val in enumerate(row_data):
            p = cells[c_idx].paragraphs[0]
            p.alignment = WD_ALIGN_PARAGRAPH.LEFT if c_idx==0 else WD_ALIGN_PARAGRAPH.CENTER
            apply_font(p.add_run(clean_val(val)), 8, bold=(r_idx==len(bkk_table_final)-1 or c_idx==0))
            if r_idx == len(bkk_table_final)-1: set_cell_background(cells[c_idx], "FFFF00")
    ...
    # Signature logic remains...
    sig_table = doc.add_table(rows=8, cols=3)
    def fill_sig(idx, lab):
        row = sig_table.rows[idx].cells
        apply_font(row[0].paragraphs[0].add_run(lab), 11, False)
        apply_font(row[1].paragraphs[0].add_run(":"), 11, False)
    fill_sig(0, "Disediakan"); fill_sig(1, "Jawatan"); fill_sig(3, "Disemak"); fill_sig(4, "Jawatan"); fill_sig(6, "Disahkan"); fill_sig(7, "Jawatan")
...
if f1 and f2:
    if st.button("🚀 Jana Laporan Lengkap"):
        try:
            ...
            df_yesterday_list = df2[df2['Tarikh Isytihar Wabak'] == yesterday][['PENYAKIT', 'DAERAH (HURUF BESAR)', addr_col, cat_col, 'Bilangan Kes', 'Bilangan Terdedah']].values.tolist()

            df2_filt = df2[df2['Tarikh Isytihar Wabak'] >= date(2026, 1, 4)].copy()
            df2_filt['PENYAKIT'] = df2_filt['PENYAKIT'].apply(lambda n: "ILI/ Influenza" if any(x in str(n).upper() for x in ["INFLUENZA", "ILI"]) else n)
            wb_sum = []
            for d in df2_filt['PENYAKIT'].unique():
                ...
                k = len(d_df)
                act = d_df.apply(lambda r: True if (pd.isna(r['Tarikh Sebenar Tamat Wabak']) or r['Tarikh Sebenar Tamat Wabak'] >= today) else False, axis=1).sum()
                wb_sum.append({'PENYAKIT': d, 'HARIAN': h, 'AKTIF': act, 'KUMULATIF': k})
            ...
            ins_semalam = df_bkk_full[df_bkk_full.iloc[:, 2].astype(str).str.contains(yesterday_str)]
            bkk_details = [{'kejadian': r[5], 'alamat': r[8], 'daerah': r[4]} for _, r in ins_semalam.iterrows()]
            bkk_raw = df_bkk_full.iloc[1:, 33:47].dropna(how='all').reset_index(drop=True)
            bkk_raw.columns = bkk_raw.iloc[0]
            bkk_table_final = bkk_raw[1:].reset_index(drop=True).rename(columns={'GOMBAK':'GBK','PETALING':'PTG','SEPANG':'SPG'})
