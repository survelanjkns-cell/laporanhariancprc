...
def generate_docx(matrix_df, col_sums, wabak_df, vector_df, bkk_table_df, is_bkk_empty, bkk_details, df_yesterday_list):
    ...
    # --- JADUAL 2.1 (NEW) ---
    doc.add_paragraph()
    add_table_title(doc, "Jadual 2.1", "Senarai Wabak Yang Dilaporkan")
    ...
    if not df_yesterday_list:
        ...
        for idx, item in enumerate(df_yesterday_list, start=1):
            ...
            for c in range(5):
                ...
                if c != 3: p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
if f1 and f2:
    if st.button("🚀 Jana Laporan Lengkap"):
        try:
            ...
            df_yesterday_list = df_yesterday[['PENYAKIT', 'DAERAH (HURUF BESAR)', addr_col]].values.tolist()
