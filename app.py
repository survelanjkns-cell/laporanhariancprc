...
def generate_docx(matrix_df, col_sums, wabak_df, vector_df, bkk_table_df, is_bkk_empty, bkk_details, yesterday_date):
    ...
    h11_text = f"1.1 Jadual di bawah menunjukkan jumlah input enotifikasi di negeri Selangor. Sejumlah {total_notifications} input notifikasi telah diterima pada {get_malay_date(yesterday_date)} dengan pecahan mengikut penyakit seperti dalam jadual 1."
    ...
    h21_text = f"2.1 Jadual di bawah menunjukkan jumlah wabak harian dan kumulatif di negeri Selangor. Sejumlah {harian_total} input notifikasi wabak diterima pada {get_malay_date(yesterday_date)}."
    ...
    for i, (penyakit, row_data) in enumerate(wabak_df.iterrows()):
        ...
        apply_font(cells[2].paragraphs[0].add_run(str(int(row_data['KUMULATIF']))), 9, bold=True)
    ...
    h31_text = f"3.1 Jadual di bawah menunjukkan jumlah wabak vektor harian dan kumulatif di negeri Selangor. Sejumlah {xx_v} input notifikasi wabak vektor telah diterima pada {get_malay_date(yesterday_date)} dengan pecahan mengikut penyakit seperti dalam jadual 3."
    ...
    t3.style = 'Table Grid'
    # (Logik t3 sama seperti skrip asal anda...)
    ...
    if is_bkk_empty:
        h41_text = f"4.1 Jadual di bawah menunjukkan jumlah kejadian insiden bencana, kecemasan dan krisis (BKK) di negeri Selangor. Tiada insiden dilaporkan pada {get_malay_date(yesterday_date)}."
    else:
        # Menukar angka ke perkataan Melayu ringkas
        num_word = {1: "satu (1)", 2: "dua (2)", 3: "tiga (3)", 4: "empat (4)", 5: "lima (5)", 6: "enam (6)", 7: "tujuh (7)", 8: "lapan (8)", 9: "sembilan (9)", 10: "sepuluh (10)"}
        count = len(bkk_details)
        count_str = num_word.get(count, f"{count} ({count})")
        
        # Susun senarai insiden
        insiden_list = []
        for item in bkk_details:
            insiden_list.append(f"kejadian {item['kejadian'].lower()} di {item['alamat']}, {item['daerah']}")
        
        # Gabung ayat dengan tanda koma dan "dan"
        ...
        h41_text = (f"4.1 Jadual di bawah menunjukkan jumlah kejadian insiden bencana, kecemasan dan krisis (BKK) di negeri Selangor. "
                    f"Terdapat {count_str} kejadian dilaporkan pada {get_malay_date(yesterday_date)} iaitu {detail_narrative}.")
    ...
    t4.style = 'Table Grid'
    # (Logik t4 sama seperti skrip asal anda...)
    ...
    if st.button("🚀 Jana Laporan Lengkap"):
        try:
            ...
            with st.spinner('Menarik data BKK...'):
                ...
                # Cari insiden mengikut tarikh di Kolum C (index 2)
                # Pastikan format tarikh di Sheets adalah DD/MM/YYYY
                insiden_semalam = df_bkk_full[df_bkk_full.iloc[:, 2].astype(str) == yesterday_str]
                
                bkk_details = []
                if not insiden_semalam.empty:
                    for _, row in insiden_semalam.iterrows():
                        bkk_details.append({
                            'kejadian': str(row[5]).strip(), # Kolum F (Kejadian)
                            'alamat': str(row[8]).strip(),   # Kolum I (Alamat)
                            'daerah': str(row[4]).strip()    # Kolum E (Daerah)
                        })
                
                is_bkk_empty = len(bkk_details) == 0
                
                # Jadual Statik di bawah Laporan 4.1
            ...
            # Generate DOCX
            doc_out = generate_docx(matrix, col_totals, wabak_df, v_data, bkk_table_final, is_bkk_empty, bkk_details, yesterday)
